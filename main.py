...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Привет! Я бот для напоминаний. Я сохраняю напоминания из канала и уведомляю о них. Используйте /upcoming для просмотра предстоящих напоминаний.")
...
Бот работает с каналом: сохраняет сообщения с #напоминание и @HH:MM DD-MM-YYYY
"""
...
async def check_reminders():
    try:
        ...
        logger.info(f"Checking {len(upcoming)} reminders")
        ...
        for note in upcoming:
            try:
                ...
                logger.info(f"✅ Sent reminder to {note.user_id}")
                
            except Exception as e:
                logger.error(f"❌ Failed to send to {note.user_id}: {e}")
...
def main():
    ...
    background_task = loop.create_task(background_tasks())
