...
def main():
    ...
    logger.info(f"Checking reminders from {now_utc} to {upper_utc}")
    try:
        reminders = get_upcoming_reminders_window(user_id=None, start_time=now_utc, end_time=upper_utc)
    ...
    for note in reminders:
        try:
            ...
            logger.info(f"Sent reminder {note.id}")
           
        except Exception as ex:
            logger.error(f"Error sending reminder {note.id}: {ex}")
